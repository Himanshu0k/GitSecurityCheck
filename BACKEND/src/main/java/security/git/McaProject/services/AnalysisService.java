...
public class AnalysisService {
    ...
    // ✅ Returns raw Gemini API response string — let the caller parse it
    public String analyzeCode(String code) {
        ...
        for (int i = 0; i < maxRetries; i++) {
            try {
                System.out.println("🧠 Gemini Analysis Attempt " + (i + 1));
                return callGemini(prompt); // ✅ Return raw string directly
                ...
                if (status == HttpStatus.SERVICE_UNAVAILABLE) {
                    long waitMs = (long) Math.pow(2, i + 1) * 1000;
                    System.out.println("⏳ Gemini 503. Attempt " + (i + 1)
                            + ". Waiting " + waitMs + "ms...");
